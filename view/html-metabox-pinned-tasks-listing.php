...
try {
  if (
    isset( $_POST['post_id'] )
    && isset( $_POST['nonce'] )
    && wp_verify_nonce( $_POST['nonce'], 'ptc_completionist_list_tasks' ) !== FALSE
  ) {
    ...
    if ( is_array( $pinned_task_gids ) && ! empty( $pinned_task_gids ) ) {
      foreach ( $pinned_task_gids as $task_gid ) {
        maybe_display_task_row( $task_gid );
      }
    } else {
...
function maybe_display_task_row( string $task_gid ) : void {
  ...
  try {
    ...
    error_log( 'Failed to fetch task data for display: ' . $e->getMessage() );
    return;
  }
