...
cd ..
touch .tmp
zip -rT9X completionist.zip .tmp completionist/ --exclude '*/.git*' '*/.DS_Store' '*.zip' '*.log' '*.map' '*.sh' README.md composer.json composer.lock composer.phar '*/node_modules/*' package-lock.json package.json terser.json 'assets/css/scss/*'
